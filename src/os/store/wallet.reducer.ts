...
import { Lamports, SPLT, Swap, WalletInterface } from '@stream-protocol/swap-js'
...
const destroyWindow = async () => {
  if (window.stream?.wallet) window.stream.wallet.disconnect()
...
export const connectWallet = createAsyncThunk(
  ...
  async (wallet: any) => {
    ...
    const lamports = await window.stream.lamports.getLamports(address)
