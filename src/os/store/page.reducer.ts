...
const troubleshoot = (register: StreamReg, appIds?: AppIds): AppIds => {
  if (!appIds || !Array.isArray(appIds)) return []
  return appIds.filter((appId) => register[appId])
}
const fetchRegister = async () => {
  try {
    const res = await fetch(StreamReg)
...
>(`${NAME}/installManifest`, async (manifest, { getState }) => {
  ...
  const newRegister: StreamReg = { ...register }
...
>(`${NAME}/loadPage`, async (_, { getState }) => {
  ...
  if (!account.isAddress(walletAddress))
    throw new Error('Wallet is not connected yet.')
  // Fetch user's apps
  const db = new PDB(walletAddress).createInstance('stream')
...
>(`${NAME}/uninstallApp`, async (appId, { getState }) => {
  ...
  const db = pdb.createInstance('stream')
  await db.setItem('appIds', newAppIds)
